...
import { useCreateTeam } from "@/components/team/CreateTeamContext";

export function UserButton() {
  const { user, currentTeam, clearUser } = useUserStore();
            ...
            >
              ...
              <div className="p-4 border-b border-border bg-sidebar-bg/50">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-sidebar-hover hover:bg-sidebar-active border border-border rounded-lg text-xs font-medium text-foreground transition-colors">
              ...
              {/* 团队信息 */}
              <div className="pt-3 px-3 pb-2 border-b border-border">
                <div className="text-xs text-muted mb-2">{t("team")}</div>
                {displayTeams.length > 0 ? (
                  <div className="space-y-0.5 mb-1.5">
                    {displayTeams.map((team) => (
                      <Link
                        ...
                        href="/settings/team"
                        onClick={closeMenu}
                        className="flex items-center gap-2.5 pl-2 pr-3 py-1.5 rounded-lg hover:bg-sidebar-hover transition-colors group"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted truncate flex items-center gap-1.5">
                            ...
                            <span>·</span>
                            <span>{t("teamMembers", { count: team.memberCount })}</span>
                          </div>
                        </div>
                        <Settings className="h-4 w-4 text-white opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 ml-2" />
                      </Link>
                    ))}
                  </div>
                ) : (
                  <Link
                    href="/settings/profile"
                    onClick={closeMenu}
                    className="flex items-center gap-2.5 p-1.5 rounded-lg hover:bg-sidebar-hover transition-colors group mb-1.5"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-foreground">
                        {user.name} {t("personalData")}
                      </div>
                      <div className="text-xs text-muted">{t("teamOwner")}</div>
                    </div>
                  </Link>
                )}
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-sidebar-hover transition-colors text-primary text-sm font-medium"
                >
