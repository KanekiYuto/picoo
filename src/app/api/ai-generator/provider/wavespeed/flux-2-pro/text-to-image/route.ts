...
// 请求参数接口
interface Flux2ProTextToImageRequest {
  prompt: string;
  size: string;
  seed?: number;
...
/**
 * POST /api/ai-generator/provider/wavespeed/flux-2-pro/text-to-image
 * Flux 2 Pro 文生图 API (异步模式，使用 webhook)
 */
export async function POST(request: NextRequest) {
  return handleWavespeedRequest(request, {
    ...
    // 参数处理回调函数
    processParams: (body: Flux2ProTextToImageRequest): ProcessParamsResult | NextResponse => {
      // 解析参数
      const {
        prompt,
        size,
      ...
      return {
        ...
        // 配额消费描述
        description: `Flux 2 Pro text-to-image: ${prompt.substring(0, 50)}...`,
      };
    },
  });
}
